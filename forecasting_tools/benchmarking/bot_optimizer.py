...
#     report_str += "<><><><><><><><><><><><><><> START TOP {x} WORST REPORTS <><><><><><><><><><><><><><>\n"
...
class BotOptimizer:
    ...
    async def create_optimized_prompt(self) -> OptimizationResult:
        ...
        for i in range(self.iterations):
            ...
            logger.info(
                f"Best survivor: {best_survivor.bot_config.reasoning_idea.short_name} with score {best_survivor.score:.4f}. Prompt:\n {best_survivor.bot_config.reasoning_prompt_template}"
            )
    ...
    async def _mutate_prompt(
        ...
        mutation_agent_task = (
            f"Generate {num_mutations_to_generate} mutated prompt ideas for the prompt named '{parent_prompt_config.reasoning_idea.short_name}'. "
        ...
        logger.info(
            f"Successfully structured {len(mutated_ideas)} mutation ideas for prompt '{parent_prompt_config.reasoning_idea.short_name}'. Requested {num_mutations_to_generate}."
        )
